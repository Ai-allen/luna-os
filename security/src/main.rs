...
fn device_domain_for_gate(target_gate:u32,domain_key:u64)->bool{match target_gate{LUNA_DEVICE_LIST|LUNA_DEVICE_CENSUS|LUNA_DEVICE_PCI_SCAN=>domain_key==LUNA_CAP_DEVICE_LIST,LUNA_DEVICE_READ|LUNA_DEVICE_QUERY|LUNA_DEVICE_INPUT_READ|LUNA_DEVICE_BLOCK_READ=>domain_key==LUNA_CAP_DEVICE_READ,LUNA_DEVICE_WRITE|LUNA_DEVICE_BLOCK_WRITE|LUNA_DEVICE_DISPLAY_PRESENT|LUNA_DEVICE_BLOCK_WRITE_INSTALL_TARGET=>domain_key==LUNA_CAP_DEVICE_WRITE,_=>false}}
...
fn validate_device_policy(gate:&mut LunaGate){let caller=gate.caller_space as u32;let actor=gate.seal_low as u32;let flags=gate.nonce;if gate.target_space!=LUNA_SPACE_DEVICE{device_deny(gate,b"target");return}if actor!=0&&actor!=caller&&caller!=LUNA_SPACE_NETWORK{device_deny(gate,b"actor");return}if !device_domain_for_gate(gate.target_gate,gate.domain_key){device_deny(gate,b"operation");return}if flags&LUNA_DEVICE_POLICY_UNSAFE_WRITE!=0&&device_unsafe_blocked(flags){device_deny(gate,b"recovery");return}let Some(index)=find_cid(gate.cid_low,gate.cid_high) else{if find_revoked_cid(gate.cid_low,gate.cid_high){device_deny(gate,b"revoked")}else{device_deny(gate,b"cap")}return};unsafe{let cid=SESSION_TABLE.get_unchecked_mut(index);if cid.live!=1||cid.domain_key!=gate.domain_key||cid.uses_left==0{device_deny(gate,b"cap");return}if cid.holder_space!=caller{device_deny(gate,b"holder");return}if cid.target_space!=0&&cid.target_space!=LUNA_SPACE_DEVICE{device_deny(gate,b"target");return}if gate.target_gate!=0&&cid.target_gate!=0&&cid.target_gate!=gate.target_gate{device_deny(gate,b"operation");return}if flags&LUNA_DEVICE_POLICY_POLL==0{cid.uses_left=cid.uses_left.saturating_sub(1);}gate.uses=cid.uses_left;}gate.status=LUNA_GATE_OK;gate.count=1;}
