...
fn approve_cap_request(gate:&mut LunaGate)->bool{if gate.domain_key==LUNA_CAP_NETWORK_PAIR{if gate.target_space==0{gate.target_space=LUNA_SPACE_NETWORK}if gate.target_space!=LUNA_SPACE_NETWORK{return false}if gate.target_gate!=0&&gate.target_gate!=LUNA_NETWORK_PAIR_PEER{return false}}if gate.domain_key==LUNA_CAP_NETWORK_SESSION{if gate.target_space==0{gate.target_space=LUNA_SPACE_NETWORK}if gate.target_space!=LUNA_SPACE_NETWORK{return false}if gate.target_gate!=0&&gate.target_gate!=LUNA_NETWORK_OPEN_SESSION&&gate.target_gate!=LUNA_NETWORK_OPEN_CHANNEL{return false}}true}
...
fn validate_cap(gate:&mut LunaGate){let Some(index)=find_cid(gate.cid_low,gate.cid_high) else{gate.status=LUNA_GATE_DENIED;gate.count=0;return};unsafe{let cid=SESSION_TABLE.get_unchecked_mut(index);if cid.live!=1||cid.domain_key!=gate.domain_key||cid.uses_left==0||(gate.caller_space!=0&&cid.holder_space!=gate.caller_space as u32)||(gate.target_space!=0&&cid.target_space!=0&&cid.target_space!=gate.target_space)||(gate.target_gate!=0&&cid.target_gate!=0&&cid.target_gate!=gate.target_gate){gate.status=LUNA_GATE_DENIED;gate.count=0;return}cid.uses_left=cid.uses_left.saturating_sub(1);gate.ttl=cid.ttl;gate.uses=cid.uses_left;}gate.status=LUNA_GATE_OK;gate.count=1;}
...
fn fold_secret(username:&[u8;16],password:&[u8;32],salt:u64)->u64{let mut value=0x9E37_79B9_7F4A_7C15u64^salt;let mut i=0usize;let user_len=fixed_string_len(username);while i<user_len{value^=username[i] as u64;value=value.rotate_left(7).wrapping_mul(0x100_0000_01B3);i+=1;}i=0;let pass_len=fixed_string_len(password);while i<pass_len{value^=password[i] as u64;value=value.rotate_left(11).wrapping_mul(0x100_0000_01B3);i+=1;}value^=((user_len as u64)<<32)|(pass_len as u64);value.rotate_left(17)^0xA5A5_5A5A_C3C3_3C3C}
fn auth_login(gate:&mut LunaGate){let Some(request)=load_auth_request(gate) else{gate.status=LUNA_GATE_DENIED;gate.count=0;return};if !consume_session_cap(gate,LUNA_CAP_USER_AUTH){gate.status=LUNA_GATE_DENIED;gate.count=0;return}let mut record=LunaUserSecretRecord{magic:0,version:0,flags:0,reserved0:0,user_low:0,user_high:0,salt:0,password_fold:0,username:[0;16]};let mut size=0u64;let secret=LunaObjectRef{low:request.secret_low,high:request.secret_high};if secret.low==0||secret.high==0||data_draw_span(secret,(&mut record as *mut LunaUserSecretRecord).cast::<u8>(),size_of::<LunaUserSecretRecord>(),&mut size)!=LUNA_DATA_OK||size<size_of::<LunaUserSecretRecord>() as u64||record.magic!=LUNA_USER_SECRET_MAGIC||record.version!=LUNA_USER_RECORD_VERSION{gate.status=LUNA_GATE_DENIED;gate.count=0;return}let req_user_len=fixed_string_len(&request.username);let rec_user_len=fixed_string_len(&record.username);if req_user_len==0||req_user_len!=rec_user_len||!bytes_equal(&request.username[..req_user_len],&record.username[..rec_user_len]){gate.status=LUNA_GATE_DENIED;gate.count=0;return}if fold_secret(&request.username,&request.password,record.salt)!=record.password_fold{gate.status=LUNA_GATE_DENIED;gate.count=0;return}gate.status=LUNA_GATE_OK;gate.count=1;}

#[unsafe(no_mangle)] pub extern "sysv64" fn security_entry_boot(_bootview:*const u8){
serial_write(b"[SECURITY] boot enter\r\n");
unsafe{GOVERN_VOLUME_STATE=LUNA_VOLUME_HEALTHY;GOVERN_SYSTEM_MODE=LUNA_MODE_NORMAL;}
