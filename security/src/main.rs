...
fn validate_network_policy(gate:&mut LunaGate){let caller=gate.caller_space as u32;let actor=gate.seal_low as u32;let flags=gate.nonce;if gate.target_space==LUNA_SPACE_DEVICE&&(flags&LUNA_NETWORK_POLICY_DEVICE_EXEC)!=0&&caller!=LUNA_SPACE_NETWORK{network_deny(gate,b"unauthorized-injection");return}if (flags&(LUNA_NETWORK_POLICY_RAW_PACKET|LUNA_NETWORK_POLICY_DEVICE_EXEC))!=0&&(flags&(LUNA_NETWORK_POLICY_DEGRADED|LUNA_NETWORK_POLICY_OFFLINE|LUNA_NETWORK_POLICY_SOFT_LOOP))!=0{network_deny(gate,b"offline");return}if caller==0{network_deny(gate,b"caller");return}if gate.target_space==LUNA_SPACE_NETWORK&&actor!=caller{network_deny(gate,b"actor");return}if gate.target_space==LUNA_SPACE_NETWORK&&gate.target_gate==LUNA_NETWORK_OPEN_CHANNEL&&gate.ttl==0{network_deny(gate,b"session");return}if gate.target_space==LUNA_SPACE_NETWORK&&(gate.target_gate==LUNA_NETWORK_SEND_CHANNEL||gate.target_gate==LUNA_NETWORK_RECV_CHANNEL)&&gate.uses==0{network_deny(gate,b"channel");return}if !network_domain_for_gate(gate.target_space,gate.target_gate,gate.domain_key){network_deny(gate,b"operation");return}let Some(index)=find_cid(gate.cid_low,gate.cid_high) else{if find_revoked_cid(gate.cid_low,gate.cid_high){network_deny(gate,b"revoked")}else{network_deny(gate,b"cap")}return};unsafe{let cid=SESSION_TABLE.get_unchecked_mut(index);if cid.live!=1||cid.domain_key!=gate.domain_key||cid.uses_left==0{network_deny(gate,b"cap");return}if cid.holder_space!=caller{network_deny(gate,b"holder");return}if gate.target_space!=0&&cid.target_space!=0&&cid.target_space!=gate.target_space{network_deny(gate,b"target");return}if gate.target_gate!=0&&cid.target_gate!=0&&cid.target_gate!=gate.target_gate{network_deny(gate,b"operation");return}cid.uses_left=cid.uses_left.saturating_sub(1);gate.ttl=cid.ttl;gate.uses=cid.uses_left;}gate.status=LUNA_GATE_OK;gate.count=1;}
...
fn network_test_gate(caller:u64,domain_key:u64,cid:LunaObjectRef,target_space:u32,target_gate:u32,flags:u64)->LunaGate{LunaGate{sequence:0,opcode:OP_VALIDATE_NETWORK,caller_space:caller,domain_key,cid_low:cid.low,cid_high:cid.high,status:0,count:0,target_space,target_gate,ttl:0,uses:0,seal_low:caller,seal_high:0,nonce:flags,buffer_addr:0,buffer_size:0}}
fn network_policy_selfcheck(){let holder_cid=issue_internal_cid(LUNA_CAP_NETWORK_SEND,LUNA_SPACE_NETWORK,LUNA_NETWORK_SEND_PACKET);let mut holder_gate=network_test_gate(LUNA_SPACE_USER as u64,LUNA_CAP_NETWORK_SEND,holder_cid,LUNA_SPACE_NETWORK,LUNA_NETWORK_SEND_PACKET,0);validate_network_policy(&mut holder_gate);if holder_gate.status==LUNA_GATE_DENIED{serial_write(b"[SECURITY] network.caller negative ok\r\n");}else{serial_write(b"[SECURITY] network.caller negative fail\r\n");}let revoked_cid=issue_internal_cid(LUNA_CAP_NETWORK_SEND,LUNA_SPACE_NETWORK,LUNA_NETWORK_SEND_PACKET);if let Some(index)=find_cid(revoked_cid.low,revoked_cid.high){unsafe{let entry=*SESSION_TABLE.get_unchecked(index);tombstone_cid(entry);*SESSION_TABLE.get_unchecked_mut(index)=EMPTY_CID;}}let mut revoked_gate=network_test_gate(LUNA_SPACE_SECURITY as u64,LUNA_CAP_NETWORK_SEND,revoked_cid,LUNA_SPACE_NETWORK,LUNA_NETWORK_SEND_PACKET,0);validate_network_policy(&mut revoked_gate);if revoked_gate.status==LUNA_GATE_DENIED{serial_write(b"[SECURITY] network.revoked negative ok\r\n");}else{serial_write(b"[SECURITY] network.revoked negative fail\r\n");}let offline_cid=issue_internal_cid(LUNA_CAP_NETWORK_SEND,LUNA_SPACE_NETWORK,LUNA_NETWORK_SEND_PACKET);let mut offline_gate=network_test_gate(LUNA_SPACE_SECURITY as u64,LUNA_CAP_NETWORK_SEND,offline_cid,LUNA_SPACE_NETWORK,LUNA_NETWORK_SEND_PACKET,LUNA_NETWORK_POLICY_RAW_PACKET|LUNA_NETWORK_POLICY_OFFLINE);validate_network_policy(&mut offline_gate);if offline_gate.status==LUNA_GATE_DENIED{serial_write(b"[SECURITY] network.offline negative ok\r\n");}else{serial_write(b"[SECURITY] network.offline negative fail\r\n");}let mut inject_gate=network_test_gate(LUNA_SPACE_PROGRAM as u64,LUNA_CAP_DEVICE_WRITE,LunaObjectRef{low:0,high:0},LUNA_SPACE_DEVICE,LUNA_DEVICE_WRITE,LUNA_NETWORK_POLICY_RAW_PACKET|LUNA_NETWORK_POLICY_DEVICE_EXEC);validate_network_policy(&mut inject_gate);if inject_gate.status==LUNA_GATE_DENIED{serial_write(b"[SECURITY] network.inject negative ok\r\n");}else{serial_write(b"[SECURITY] network.inject negative fail\r\n");}}
