...
const OP_VALIDATE_NETWORK: u32 = 18;
const OP_VALIDATE_DEVICE: u32 = 19;
...
const LUNA_DEVICE_LIST: u32 = 1;
const LUNA_DEVICE_READ: u32 = 2;
const LUNA_DEVICE_WRITE: u32 = 3;
const LUNA_DEVICE_CENSUS: u32 = 4;
const LUNA_DEVICE_PCI_SCAN: u32 = 5;
const LUNA_DEVICE_QUERY: u32 = 6;
const LUNA_DEVICE_INPUT_READ: u32 = 7;
const LUNA_DEVICE_BLOCK_READ: u32 = 8;
const LUNA_NETWORK_PAIR_PEER: u32 = 3;
const LUNA_NETWORK_SEND_PACKET: u32 = 1;
const LUNA_NETWORK_RECV_PACKET: u32 = 2;
const LUNA_NETWORK_OPEN_SESSION: u32 = 4;
const LUNA_NETWORK_OPEN_CHANNEL: u32 = 5;
const LUNA_NETWORK_SEND_CHANNEL: u32 = 6;
const LUNA_NETWORK_RECV_CHANNEL: u32 = 7;
const LUNA_NETWORK_GET_INFO: u32 = 8;
const LUNA_NETWORK_POLICY_RAW_PACKET: u64 = 1u64 << 0;
const LUNA_NETWORK_POLICY_DEGRADED: u64 = 1u64 << 1;
const LUNA_NETWORK_POLICY_OFFLINE: u64 = 1u64 << 2;
const LUNA_NETWORK_POLICY_SOFT_LOOP: u64 = 1u64 << 3;
const LUNA_NETWORK_POLICY_DEVICE_EXEC: u64 = 1u64 << 4;
const LUNA_DEVICE_POLICY_READ: u64 = 1u64 << 0;
const LUNA_DEVICE_POLICY_WRITE: u64 = 1u64 << 1;
const LUNA_DEVICE_POLICY_CONTROL: u64 = 1u64 << 2;
const LUNA_DEVICE_POLICY_UNSAFE_WRITE: u64 = 1u64 << 3;
const LUNA_DEVICE_POLICY_DEGRADED: u64 = 1u64 << 4;
const LUNA_DEVICE_POLICY_RECOVERY: u64 = 1u64 << 5;
const LUNA_DEVICE_POLICY_POLL: u64 = 1u64 << 6;
const SPACE_CAPACITY: usize = 128;
const SEAL_CAPACITY: usize = 64;
const REVOKED_CAPACITY: usize = 16;
...
const LUNA_OBSERVE_LOG: u32 = 1;
const LUNA_OBSERVE_OK: u32 = 0;
...
const LUNA_DATA_OBJECT_TYPE_LSON_RECORD: u32 = 0x4C534F52;
const LUNA_LSON_MAGIC: u32 = 0x4C53_4F4E;
const LUNA_LSON_VERSION: u32 = 0x0001_0000;
const LUNA_LOG_CLASS_SECURITY: u32 = 6;
const LUNA_LOG_CLASS_CRYPTO: u32 = 7;
const LUNA_LOG_BAND_AUDIT: u32 = 8;
const LUNA_LSON_ENCODING_FRAME: u32 = 2;
const LUNA_LSON_FLAG_PERSIST: u32 = 1u32 << 0;
const LUNA_LSON_FLAG_AUDIT: u32 = 1u32 << 1;
...
const LUNA_DEVICE_ID_DISK0: u32 = 2;
const LUNA_DEVICE_ID_INPUT0: u32 = 5;
const LUNA_DEVICE_ID_NET0: u32 = 6;
const LUNA_DEVICE_ID_POINTER0: u32 = 7;
const LUNA_DEVICE_ID_DISK1: u32 = 8;
...
#[repr(C)] struct LunaObserveGate{sequence:u32,opcode:u32,status:u32,space_id:u32,level:u32,result_count:u32,cid_low:u64,cid_high:u64,start_time:u64,end_time:u64,buffer_addr:u64,buffer_size:u64,message:[u8;32]}
#[repr(C)] #[derive(Copy,Clone)] struct LunaLsonRecord{magic:u32,version:u32,tick:u64,actor_low:u64,actor_high:u64,trace_low:u64,trace_high:u64,session_low:u64,session_high:u64,install_low:u64,install_high:u64,record_class:u32,band:u32,space_id:u32,writer_space:u32,authority_space:u32,encoding:u32,flags:u32,device_id:u32,driver_family:u32,attr_count:u32,reserved0:u32,kind:[u8;24],scope:[u8;16],body:[u8;48]}
...
const EMPTY_LSON_RECORD:LunaLsonRecord=LunaLsonRecord{magic:0,version:0,tick:0,actor_low:0,actor_high:0,trace_low:0,trace_high:0,session_low:0,session_high:0,install_low:0,install_high:0,record_class:0,band:0,space_id:0,writer_space:0,authority_space:0,encoding:0,flags:0,device_id:0,driver_family:0,attr_count:0,reserved0:0,kind:[0;24],scope:[0;16],body:[0;48]};
...
static mut REVOKED_TABLE:[SessionCid;REVOKED_CAPACITY]=[EMPTY_CID;REVOKED_CAPACITY];
static mut SEAL_TABLE:[InvokeSeal;SEAL_CAPACITY]=[EMPTY_SEAL;SEAL_CAPACITY];
static mut LINK_SESSION_OWNER:u32=0;
static mut LINK_CHANNEL_OWNER:u32=0;
static mut LINK_SESSION_ID:u32=0;
static mut LINK_CHANNEL_ID:u32=0;
static mut LINK_CHANNEL_SESSION:u32=0;
...
static mut OBSERVE_LOG_CID:LunaObjectRef=LunaObjectRef{low:0,high:0};
static mut SECURITY_AUDIT_PENDING:[LunaLsonRecord;16]=[EMPTY_LSON_RECORD;16];
static mut SECURITY_AUDIT_PENDING_COUNT:usize=0;
static mut OBSERVE_LSON_READY:u32=0;
...
fn call_observe_gate(entry:u64,gate:*mut LunaObserveGate){let func:extern "sysv64" fn(*mut LunaObserveGate)=unsafe{transmute(entry as usize)};func(gate);}
fn copy_text<const N:usize>(dst:&mut [u8;N],src:&[u8]){let mut i=0usize;while i<N{dst[i]=0;i+=1;}i=0;while i+1<N&&i<src.len(){let b=src[i];if b==0{break}dst[i]=b;i+=1;}}
fn security_audit_emit(record:&LunaLsonRecord)->bool{let cid=unsafe{OBSERVE_LOG_CID};if cid.low==0{return false}let m=manifest();if m.observe_gate_entry==0||m.observe_gate_base==0{return false}let gate=m.observe_gate_base as *mut LunaObserveGate;zero_bytes(gate.cast::<u8>(),size_of::<LunaObserveGate>());unsafe{(*gate).sequence=301;(*gate).opcode=LUNA_OBSERVE_LOG;(*gate).space_id=LUNA_SPACE_SECURITY;(*gate).level=LUNA_LOG_BAND_AUDIT;(*gate).cid_low=cid.low;(*gate).cid_high=cid.high;(*gate).buffer_addr=(record as *const LunaLsonRecord) as u64;(*gate).buffer_size=size_of::<LunaLsonRecord>() as u64;copy_text(&mut (*gate).message,&record.body);call_observe_gate(m.observe_gate_entry,gate);(*gate).status==LUNA_OBSERVE_OK}}
fn security_audit_flush(){if unsafe{OBSERVE_LSON_READY}==0{return}let count=unsafe{SECURITY_AUDIT_PENDING_COUNT};let mut i=0usize;while i<count&&i<unsafe{SECURITY_AUDIT_PENDING.len()}{let record=unsafe{SECURITY_AUDIT_PENDING[i]};if !security_audit_emit(&record){return}i+=1;}unsafe{SECURITY_AUDIT_PENDING_COUNT=0;}}
fn security_audit_queue(record:LunaLsonRecord){if unsafe{OBSERVE_LSON_READY}!=0{if security_audit_emit(&record){return}}let count=unsafe{SECURITY_AUDIT_PENDING_COUNT};if count<unsafe{SECURITY_AUDIT_PENDING.len()}{unsafe{SECURITY_AUDIT_PENDING[count]=record;SECURITY_AUDIT_PENDING_COUNT=count+1;}}else{unsafe{SECURITY_AUDIT_PENDING[0]=record;}}}
fn security_audit_lson(record_class:u32,kind:&[u8],scope:&[u8],body:&[u8],gate:&LunaGate){let m=manifest();let mut record=EMPTY_LSON_RECORD;record.magic=LUNA_LSON_MAGIC;record.version=LUNA_LSON_VERSION;record.actor_low=gate.caller_space;record.trace_low=gate.domain_key;record.session_low=gate.cid_low;record.session_high=gate.cid_high;record.install_low=m.install_uuid_low;record.install_high=m.install_uuid_high;record.record_class=record_class;record.band=LUNA_LOG_BAND_AUDIT;record.space_id=LUNA_SPACE_SECURITY;record.writer_space=LUNA_SPACE_SECURITY;record.authority_space=LUNA_SPACE_OBSERVABILITY;record.encoding=LUNA_LSON_ENCODING_FRAME;record.flags=LUNA_LSON_FLAG_PERSIST|LUNA_LSON_FLAG_AUDIT;record.attr_count=3;copy_text(&mut record.kind,kind);copy_text(&mut record.scope,scope);copy_text(&mut record.body,body);security_audit_queue(record);}
fn mark_observe_lson_ready(){unsafe{OBSERVE_LSON_READY=1;}security_audit_flush();}
...
fn find_revoked_cid(low:u64,high:u64)->bool{let mut i=0usize;while i<REVOKED_CAPACITY{let e=unsafe{*REVOKED_TABLE.get_unchecked(i)};if e.live==2&&e.cid_low==low&&e.cid_high==high{return true}i+=1;}false}
fn tombstone_cid(mut entry:SessionCid){entry.live=2;let mut i=0usize;while i<REVOKED_CAPACITY{if unsafe{(*REVOKED_TABLE.get_unchecked(i)).live}==0{unsafe{*REVOKED_TABLE.get_unchecked_mut(i)=entry;}return}i+=1;}unsafe{*REVOKED_TABLE.get_unchecked_mut(0)=entry;}}
...
fn cap_issue_deny_reason(gate:&LunaGate)->Option<&'static [u8]>{let caller=gate.caller_space as u32;if gate.domain_key==LUNA_CAP_USER_AUTH&&caller!=LUNA_SPACE_USER{return Some(b"user-auth-caller")}if matches!(gate.domain_key,LUNA_CAP_DEVICE_BIND|LUNA_CAP_DEVICE_MMIO|LUNA_CAP_DEVICE_DMA|LUNA_CAP_DEVICE_IRQ)&&caller!=LUNA_SPACE_DEVICE&&caller!=LUNA_SPACE_SECURITY{return Some(b"high-risk-caller")}None}
...
fn issue_session_cid(gate:&mut LunaGate){
if let Some(reason)=cap_issue_deny_reason(gate){
serial_write(b"audit cap.issue denied reason=");serial_write(reason);serial_write(b"\r\n");
if reason==b"user-auth-caller"{security_audit_lson(LUNA_LOG_CLASS_SECURITY,b"cap.issue",reason,b"cap.issue deny user-auth",gate);}
if reason==b"high-risk-caller"{security_audit_lson(LUNA_LOG_CLASS_SECURITY,b"cap.issue",reason,b"cap.issue deny high-risk",gate);}
gate.status=LUNA_GATE_DENIED;gate.count=0;return}
if !approve_cap_request(gate){gate.status=LUNA_GATE_DENIED;gate.count=0;return}
let Some(spec)=find_spec(gate.domain_key) else{gate.status=LUNA_GATE_UNKNOWN;gate.count=0;return};
let Some(index)=alloc_cid_slot() else{gate.status=LUNA_GATE_EXHAUSTED;gate.count=0;return};
let ttl=if gate.ttl==0{DEFAULT_TTL}else{gate.ttl};
let uses=if gate.uses==0{DEFAULT_USES}else{gate.uses};
let nonce=next_nonce();
let caller=gate.caller_space as u32;
let cid_low=spec.base_low^nonce.rotate_left(7)^((index as u64)<<24);
let cid_high=spec.base_high^nonce.rotate_right(11)^((caller as u64)<<32);
let out_sequence=gate.sequence;let out_opcode=gate.opcode;let out_caller_space=gate.caller_space;let out_domain_key=gate.domain_key;let out_target_space=gate.target_space;let out_target_gate=gate.target_gate;let out_seal_low=gate.seal_low;let out_seal_high=gate.seal_high;let out_buffer_addr=gate.buffer_addr;let out_buffer_size=gate.buffer_size;
unsafe{*SESSION_TABLE.get_unchecked_mut(index)=SessionCid{live:1,holder_space:caller,target_space:gate.target_space,target_gate:gate.target_gate,domain_key:spec.domain_key,cid_low,cid_high,ttl,uses_left:uses};}
gate.cid_low=cid_low;gate.cid_high=cid_high;gate.ttl=ttl;gate.uses=uses;gate.nonce=nonce;gate.status=LUNA_GATE_OK;gate.count=1;
if out_caller_space as u32==LUNA_SPACE_USER&&out_domain_key==LUNA_CAP_OBSERVE_READ{mark_observe_lson_ready();gate.sequence=out_sequence;gate.opcode=out_opcode;gate.caller_space=out_caller_space;gate.domain_key=out_domain_key;gate.cid_low=cid_low;gate.cid_high=cid_high;gate.target_space=out_target_space;gate.target_gate=out_target_gate;gate.ttl=ttl;gate.uses=uses;gate.seal_low=out_seal_low;gate.seal_high=out_seal_high;gate.nonce=nonce;gate.buffer_addr=out_buffer_addr;gate.buffer_size=out_buffer_size;gate.status=LUNA_GATE_OK;gate.count=1;}
}
...
fn network_domain_for_gate(target_space:u32,target_gate:u32,domain_key:u64)->bool{if target_space==LUNA_SPACE_DEVICE{return (domain_key==LUNA_CAP_DEVICE_WRITE&&target_gate==LUNA_DEVICE_WRITE)||(domain_key==LUNA_CAP_DEVICE_READ&&target_gate==LUNA_DEVICE_READ)}match target_gate{LUNA_NETWORK_SEND_PACKET|LUNA_NETWORK_SEND_CHANNEL=>domain_key==LUNA_CAP_NETWORK_SEND,LUNA_NETWORK_RECV_PACKET|LUNA_NETWORK_RECV_CHANNEL|LUNA_NETWORK_GET_INFO=>domain_key==LUNA_CAP_NETWORK_RECV,LUNA_NETWORK_PAIR_PEER=>domain_key==LUNA_CAP_NETWORK_PAIR,LUNA_NETWORK_OPEN_SESSION|LUNA_NETWORK_OPEN_CHANNEL=>domain_key==LUNA_CAP_NETWORK_SESSION,_=>false}}
fn network_deny(gate:&mut LunaGate,reason:&[u8]){serial_write(b"audit network.policy denied reason=");serial_write(reason);serial_write(b"\r\n");if reason==b"revoked"{security_audit_lson(LUNA_LOG_CLASS_SECURITY,b"network.policy",reason,b"network.policy deny revoked",gate);}gate.status=LUNA_GATE_DENIED;gate.count=0;}
fn network_link_gate(target_gate:u32)->bool{target_gate==LUNA_NETWORK_OPEN_SESSION||target_gate==LUNA_NETWORK_OPEN_CHANNEL||target_gate==LUNA_NETWORK_SEND_CHANNEL||target_gate==LUNA_NETWORK_RECV_CHANNEL}
fn link_validate_binding(gate:&mut LunaGate,caller:u32)->bool{if gate.target_space!=LUNA_SPACE_NETWORK{return true}match gate.target_gate{LUNA_NETWORK_OPEN_SESSION=>{if gate.ttl!=0{unsafe{LINK_SESSION_OWNER=caller;LINK_SESSION_ID=gate.ttl;}}true},LUNA_NETWORK_OPEN_CHANNEL=>{if gate.ttl==0||unsafe{LINK_SESSION_OWNER!=caller||LINK_SESSION_ID!=gate.ttl}{network_deny(gate,b"session");return false}if gate.uses!=0{unsafe{LINK_CHANNEL_OWNER=caller;LINK_CHANNEL_ID=gate.uses;LINK_CHANNEL_SESSION=gate.ttl;}}true},LUNA_NETWORK_SEND_CHANNEL|LUNA_NETWORK_RECV_CHANNEL=>{if gate.uses==0||unsafe{LINK_CHANNEL_OWNER!=caller||LINK_CHANNEL_ID!=gate.uses||(gate.ttl!=0&&LINK_CHANNEL_SESSION!=gate.ttl)}{network_deny(gate,b"channel");return false}true},_=>true}}
fn validate_network_policy(gate:&mut LunaGate){let caller=gate.caller_space as u32;let actor=gate.seal_low as u32;let flags=gate.nonce;if gate.target_space==LUNA_SPACE_DEVICE&&(flags&LUNA_NETWORK_POLICY_DEVICE_EXEC)!=0&&caller!=LUNA_SPACE_NETWORK{network_deny(gate,b"unauthorized-injection");return}if (flags&(LUNA_NETWORK_POLICY_RAW_PACKET|LUNA_NETWORK_POLICY_DEVICE_EXEC))!=0&&(flags&(LUNA_NETWORK_POLICY_DEGRADED|LUNA_NETWORK_POLICY_OFFLINE|LUNA_NETWORK_POLICY_SOFT_LOOP))!=0{network_deny(gate,b"offline");return}if network_link_gate(gate.target_gate)&&(flags&(LUNA_NETWORK_POLICY_DEGRADED|LUNA_NETWORK_POLICY_OFFLINE))!=0{network_deny(gate,b"link-offline");return}if caller==0{network_deny(gate,b"caller");return}if gate.target_space==LUNA_SPACE_NETWORK&&actor!=caller{network_deny(gate,b"actor");return}if !network_domain_for_gate(gate.target_space,gate.target_gate,gate.domain_key){network_deny(gate,b"operation");return}let Some(index)=find_cid(gate.cid_low,gate.cid_high) else{if find_revoked_cid(gate.cid_low,gate.cid_high){network_deny(gate,b"revoked")}else{network_deny(gate,b"cap")}return};let cid=unsafe{*SESSION_TABLE.get_unchecked(index)};if cid.live!=1||cid.domain_key!=gate.domain_key||cid.uses_left==0{network_deny(gate,b"cap");return}if cid.holder_space!=caller{network_deny(gate,b"holder");return}if gate.target_space!=0&&cid.target_space!=0&&cid.target_space!=gate.target_space{network_deny(gate,b"target");return}if gate.target_gate!=0&&cid.target_gate!=0&&cid.target_gate!=gate.target_gate{network_deny(gate,b"operation");return}if !link_validate_binding(gate,caller){return}unsafe{let cid=SESSION_TABLE.get_unchecked_mut(index);cid.uses_left=cid.uses_left.saturating_sub(1);gate.ttl=cid.ttl;gate.uses=cid.uses_left;}gate.status=LUNA_GATE_OK;gate.count=1;}
fn device_expected_domain(target_gate:u32)->u64{match target_gate{LUNA_DEVICE_LIST|LUNA_DEVICE_CENSUS|LUNA_DEVICE_PCI_SCAN=>LUNA_CAP_DEVICE_LIST,LUNA_DEVICE_READ|LUNA_DEVICE_QUERY|LUNA_DEVICE_INPUT_READ|LUNA_DEVICE_BLOCK_READ=>LUNA_CAP_DEVICE_READ,LUNA_DEVICE_WRITE|LUNA_DEVICE_BLOCK_WRITE|LUNA_DEVICE_DISPLAY_PRESENT|LUNA_DEVICE_BLOCK_WRITE_INSTALL_TARGET=>LUNA_CAP_DEVICE_WRITE,_=>0}}
fn device_required_flags(target_gate:u32,device_id:u32,provided:u64)->u64{let mut flags=match target_gate{LUNA_DEVICE_LIST|LUNA_DEVICE_CENSUS|LUNA_DEVICE_PCI_SCAN=>LUNA_DEVICE_POLICY_CONTROL,LUNA_DEVICE_READ|LUNA_DEVICE_QUERY|LUNA_DEVICE_BLOCK_READ=>LUNA_DEVICE_POLICY_READ,LUNA_DEVICE_INPUT_READ=>LUNA_DEVICE_POLICY_READ|LUNA_DEVICE_POLICY_POLL,LUNA_DEVICE_WRITE|LUNA_DEVICE_DISPLAY_PRESENT=>LUNA_DEVICE_POLICY_WRITE,LUNA_DEVICE_BLOCK_WRITE|LUNA_DEVICE_BLOCK_WRITE_INSTALL_TARGET=>LUNA_DEVICE_POLICY_WRITE|LUNA_DEVICE_POLICY_UNSAFE_WRITE,_=>0};if target_gate==LUNA_DEVICE_READ&&(device_id==LUNA_DEVICE_ID_INPUT0||device_id==LUNA_DEVICE_ID_POINTER0){flags|=LUNA_DEVICE_POLICY_POLL;}if target_gate==LUNA_DEVICE_WRITE&&(device_id==LUNA_DEVICE_ID_DISK0||device_id==LUNA_DEVICE_ID_DISK1){flags|=LUNA_DEVICE_POLICY_UNSAFE_WRITE;}flags|provided}
fn device_domain_for_gate(target_gate:u32,domain_key:u64)->bool{let expected=device_expected_domain(target_gate);expected!=0&&domain_key==expected}
fn device_requires_network_owner(target_gate:u32,device_id:u32)->bool{device_id==LUNA_DEVICE_ID_NET0&&(target_gate==LUNA_DEVICE_READ||target_gate==LUNA_DEVICE_WRITE||target_gate==LUNA_DEVICE_QUERY)}
fn device_deny(gate:&mut LunaGate,reason:&[u8]){if reason!=b"cap"{serial_write(b"audit device.policy denied reason=");serial_write(reason);serial_write(b"\r\n");}if reason==b"revoked"{security_audit_lson(LUNA_LOG_CLASS_SECURITY,b"device.policy",reason,b"device.policy deny revoked",gate);}gate.status=LUNA_GATE_DENIED;gate.count=0;}
fn device_unsafe_blocked(flags:u64)->bool{if flags&(LUNA_DEVICE_POLICY_DEGRADED|LUNA_DEVICE_POLICY_RECOVERY)!=0{return true}let mode=unsafe{GOVERN_SYSTEM_MODE};let volume=unsafe{GOVERN_VOLUME_STATE};mode==LUNA_MODE_READONLY||mode==LUNA_MODE_RECOVERY||mode==LUNA_MODE_FATAL||volume==LUNA_VOLUME_RECOVERY_REQUIRED||volume==LUNA_VOLUME_FATAL_INCOMPATIBLE||volume==LUNA_VOLUME_FATAL_UNRECOVERABLE}
fn validate_device_policy(gate:&mut LunaGate){let caller=gate.caller_space as u32;let actor=gate.seal_low as u32;let device_id=gate.ttl;let domain_key=if gate.domain_key==0{device_expected_domain(gate.target_gate)}else{gate.domain_key};let flags=device_required_flags(gate.target_gate,device_id,gate.nonce);gate.domain_key=domain_key;gate.nonce=flags;if gate.target_space!=LUNA_SPACE_DEVICE{device_deny(gate,b"target");return}if actor!=0&&actor!=caller&&caller!=LUNA_SPACE_NETWORK{device_deny(gate,b"actor");return}if device_requires_network_owner(gate.target_gate,device_id)&&caller!=LUNA_SPACE_NETWORK{device_deny(gate,b"net-owner");return}if !device_domain_for_gate(gate.target_gate,domain_key){device_deny(gate,b"operation");return}if flags&LUNA_DEVICE_POLICY_UNSAFE_WRITE!=0&&device_unsafe_blocked(flags){device_deny(gate,b"recovery");return}let Some(index)=find_cid(gate.cid_low,gate.cid_high) else{if find_revoked_cid(gate.cid_low,gate.cid_high){device_deny(gate,b"revoked")}else{device_deny(gate,b"cap")}return};unsafe{let cid=SESSION_TABLE.get_unchecked_mut(index);if cid.live!=1||cid.domain_key!=domain_key||cid.uses_left==0{device_deny(gate,b"cap");return}if cid.holder_space!=caller{device_deny(gate,b"holder");return}if cid.target_space!=0&&cid.target_space!=LUNA_SPACE_DEVICE{device_deny(gate,b"target");return}if gate.target_gate!=0&&cid.target_gate!=0&&cid.target_gate!=gate.target_gate{device_deny(gate,b"operation");return}if flags&LUNA_DEVICE_POLICY_POLL==0{cid.uses_left=cid.uses_left.saturating_sub(1);}gate.uses=cid.uses_left;}gate.status=LUNA_GATE_OK;gate.count=1;}
...
fn revoke_cap(gate:&mut LunaGate){let Some(index)=find_cid(gate.cid_low,gate.cid_high) else{gate.status=LUNA_GATE_DENIED;gate.count=0;return};unsafe{let entry=*SESSION_TABLE.get_unchecked(index);if gate.caller_space as u32!=entry.holder_space&&gate.caller_space as u32!=LUNA_SPACE_SECURITY{serial_write(b"audit cap.revoke denied reason=holder\r\n");security_audit_lson(LUNA_LOG_CLASS_SECURITY,b"cap.revoke",b"holder",b"cap.revoke deny holder",gate);gate.status=LUNA_GATE_DENIED;gate.count=0;return}tombstone_cid(entry);*SESSION_TABLE.get_unchecked_mut(index)=EMPTY_CID;}gate.status=LUNA_GATE_OK;gate.count=1;}
fn revoke_domain(gate:&mut LunaGate){let mut revoked=0u32;let holder=gate.caller_space as u32;let mut i=0usize;while i<SPACE_CAPACITY{let entry=unsafe{*SESSION_TABLE.get_unchecked(i)};if entry.live==1&&entry.holder_space==holder&&entry.domain_key==gate.domain_key{tombstone_cid(entry);unsafe{*SESSION_TABLE.get_unchecked_mut(i)=EMPTY_CID;}revoked=revoked.wrapping_add(1);}i+=1;}gate.status=LUNA_GATE_OK;gate.count=revoked;}
...
fn crypto_scope_allowed(caller:u32,key_class:u32)->bool{match key_class{LUNA_CRYPTO_KEY_USER_AUTH=>caller==LUNA_SPACE_USER||caller==LUNA_SPACE_SECURITY,LUNA_CRYPTO_KEY_TRUST|LUNA_CRYPTO_KEY_LDAT=>caller==LUNA_SPACE_SECURITY,_=>false}}
fn consume_crypto_cap(gate:&mut LunaGate)->bool{let Some(index)=find_cid(gate.cid_low,gate.cid_high) else{return false};unsafe{let cid=SESSION_TABLE.get_unchecked_mut(index);if cid.live!=1||cid.domain_key!=LUNA_CAP_USER_AUTH||cid.uses_left==0||(gate.caller_space!=0&&cid.holder_space!=gate.caller_space as u32){return false}if cid.target_space!=0&&cid.target_space!=LUNA_SPACE_SECURITY{return false}if cid.target_gate!=0&&cid.target_gate!=OP_CRYPTO_SECRET{return false}cid.uses_left=cid.uses_left.saturating_sub(1);gate.ttl=cid.ttl;gate.uses=cid.uses_left;}true}
fn crypto_secret(gate:&mut LunaGate){let Some(mut request)=load_crypto_secret_request(gate) else{gate.status=LUNA_GATE_DENIED;gate.count=0;return};if !crypto_scope_allowed(gate.caller_space as u32,request.key_class){serial_write(b"audit crypto.secret denied reason=scope\r\n");security_audit_lson(LUNA_LOG_CLASS_CRYPTO,b"crypto.secret",b"scope",b"crypto.secret deny scope",gate);gate.status=LUNA_GATE_DENIED;gate.count=0;return}if !consume_crypto_cap(gate){serial_write(b"audit crypto.secret denied reason=cap\r\n");security_audit_lson(LUNA_LOG_CLASS_CRYPTO,b"crypto.secret",b"cap",b"crypto.secret deny cap",gate);gate.status=LUNA_GATE_DENIED;gate.count=0;return}let (low,high)=derive_secret_pair(request.key_class,request.subject_low,request.subject_high,&request.username,&request.secret,request.salt);request.output_low=low;request.output_high=high;store_crypto_secret_request(gate,&request);serial_write(b"audit crypto.secret release\r\n");gate.status=LUNA_GATE_OK;gate.count=1;}
fn trust_eval_deny(gate:&mut LunaGate,request:&mut LunaTrustEvalRequest,state:u32,reason:&[u8],body:&[u8]){if state!=0{request.result_state=state;store_trust_request(gate,request);}serial_write(b"audit trust.eval denied reason=");serial_write(reason);serial_write(b"\r\n");security_audit_lson(LUNA_LOG_CLASS_SECURITY,b"trust.eval",reason,body,gate);gate.status=LUNA_GATE_DENIED;gate.count=0;}
fn trust_eval(gate:&mut LunaGate){let Some(mut request)=load_trust_request(gate) else{gate.status=LUNA_GATE_DENIED;gate.count=0;return};if !consume_session_cap(gate,LUNA_CAP_PACKAGE_INSTALL){trust_eval_deny(gate,&mut request,0,b"cap",b"trust.eval deny cap");return}if request.min_proto_version!=0&&(request.min_proto_version>LUNA_PROTO_VERSION||(request.max_proto_version!=0&&request.max_proto_version<LUNA_PROTO_VERSION)){trust_eval_deny(gate,&mut request,LUNA_TRUST_RESULT_PROTO,b"proto",b"trust.eval deny proto");return}request.result_integrity=compute_bundle_integrity(request.blob_addr,request.blob_size,request.header_bytes);request.result_signature=compute_bundle_signature(&request);if request.integrity_check!=request.result_integrity{trust_eval_deny(gate,&mut request,LUNA_TRUST_RESULT_INTEGRITY,b"integrity",b"trust.eval deny integrity");return}if request.source_id!=0&&(request.signer_id==0||request.signature_check==0||request.signature_check!=request.result_signature){trust_eval_deny(gate,&mut request,LUNA_TRUST_RESULT_SIGNATURE,b"signature",b"trust.eval deny signature");return}if request.signer_id!=0{let mut signer=EMPTY_POLICY_QUERY;let mut source=EMPTY_POLICY_QUERY;if !policy_lookup(LUNA_POLICY_TYPE_SIGNER,request.signer_id,&mut signer)||signer.state!=LUNA_POLICY_STATE_ALLOW{trust_eval_deny(gate,&mut request,LUNA_TRUST_RESULT_SIGNER,b"signer",b"trust.eval deny signer");return}if !policy_lookup(LUNA_POLICY_TYPE_SOURCE,request.source_id,&mut source)||source.state!=LUNA_POLICY_STATE_ALLOW{trust_eval_deny(gate,&mut request,LUNA_TRUST_RESULT_SOURCE,b"source",b"trust.eval deny source");return}if source.binding_id!=0&&source.binding_id!=request.signer_id{request.binding_id=source.binding_id;trust_eval_deny(gate,&mut request,LUNA_TRUST_RESULT_BINDING,b"binding",b"trust.eval deny binding");return}request.binding_id=source.binding_id;}else{request.binding_id=0;}request.result_state=LUNA_TRUST_RESULT_ALLOW;store_trust_request(gate,&request);serial_write(b"audit trust.eval allow\r\n");gate.status=LUNA_GATE_OK;gate.count=1;}
...
fn crypto_test_gate(caller:u64,cid:LunaObjectRef,request:&mut LunaCryptoSecretRequest)->LunaGate{LunaGate{sequence:0,opcode:OP_CRYPTO_SECRET,caller_space:caller,domain_key:LUNA_CAP_USER_AUTH,cid_low:cid.low,cid_high:cid.high,status:0,count:0,target_space:LUNA_SPACE_SECURITY,target_gate:OP_CRYPTO_SECRET,ttl:0,uses:0,seal_low:0,seal_high:0,nonce:0,buffer_addr:(request as *mut LunaCryptoSecretRequest) as u64,buffer_size:size_of::<LunaCryptoSecretRequest>() as u64}}
fn cap_issue_test_gate(caller:u64,domain_key:u64)->LunaGate{LunaGate{sequence:0,opcode:OP_REQUEST_CAP,caller_space:caller,domain_key,cid_low:0,cid_high:0,status:0,count:0,target_space:0,target_gate:0,ttl:0,uses:0,seal_low:0,seal_high:0,nonce:0,buffer_addr:0,buffer_size:0}}
fn security_crypto_cap_selfcheck(){let mut scope_request=EMPTY_CRYPTO_SECRET_REQUEST;scope_request.key_class=LUNA_CRYPTO_KEY_TRUST;let mut scope_gate=crypto_test_gate(LUNA_SPACE_USER as u64,LunaObjectRef{low:0,high:0},&mut scope_request);crypto_secret(&mut scope_gate);if scope_gate.status==LUNA_GATE_DENIED{serial_write(b"[SECURITY] crypto.scope negative ok\r\n");}else{serial_write(b"[SECURITY] crypto.scope negative fail\r\n");}let mut authcap_gate=cap_issue_test_gate(LUNA_SPACE_SYSTEM as u64,LUNA_CAP_USER_AUTH);issue_session_cid(&mut authcap_gate);if authcap_gate.status==LUNA_GATE_DENIED{serial_write(b"[SECURITY] crypto.authcap negative ok\r\n");}else{serial_write(b"[SECURITY] crypto.authcap negative fail\r\n");}let bad_target_cid=issue_internal_cid(LUNA_CAP_USER_AUTH,LUNA_SPACE_DATA,OP_CRYPTO_SECRET);let mut target_request=EMPTY_CRYPTO_SECRET_REQUEST;target_request.key_class=LUNA_CRYPTO_KEY_USER_AUTH;target_request.subject_low=0x1000;target_request.subject_high=0x2000;let mut target_gate=crypto_test_gate(LUNA_SPACE_SECURITY as u64,bad_target_cid,&mut target_request);crypto_secret(&mut target_gate);if target_gate.status==LUNA_GATE_DENIED{serial_write(b"[SECURITY] crypto.target negative ok\r\n");}else{serial_write(b"[SECURITY] crypto.target negative fail\r\n");}let revoke_cid=issue_internal_cid(LUNA_CAP_NETWORK_SEND,LUNA_SPACE_NETWORK,LUNA_NETWORK_SEND_PACKET);let mut revoke_gate=LunaGate{sequence:0,opcode:OP_REVOKE_CAP,caller_space:LUNA_SPACE_USER as u64,domain_key:LUNA_CAP_NETWORK_SEND,cid_low:revoke_cid.low,cid_high:revoke_cid.high,status:0,count:0,target_space:0,target_gate:0,ttl:0,uses:0,seal_low:0,seal_high:0,nonce:0,buffer_addr:0,buffer_size:0};revoke_cap(&mut revoke_gate);if revoke_gate.status==LUNA_GATE_DENIED{serial_write(b"[SECURITY] cap.revoke holder negative ok\r\n");}else{serial_write(b"[SECURITY] cap.revoke holder negative fail\r\n");}let mut highrisk_gate=cap_issue_test_gate(LUNA_SPACE_USER as u64,LUNA_CAP_DEVICE_DMA);issue_session_cid(&mut highrisk_gate);if highrisk_gate.status==LUNA_GATE_DENIED{serial_write(b"[SECURITY] cap.highrisk negative ok\r\n");}else{serial_write(b"[SECURITY] cap.highrisk negative fail\r\n");}let mut protect_query=EMPTY_GOVERNANCE_QUERY;protect_query.action=LUNA_GOVERN_WRITE;protect_query.writer_space=LUNA_SPACE_USER;protect_query.object_type=LUNA_DATA_OBJECT_TYPE_USER_SECRET;protect_query.object_flags=LUNA_USER_SECRET_FLAG_SECURITY_OWNED|LUNA_USER_SECRET_FLAG_INSTALL_BOUND;let mut protect_gate=LunaGate{sequence:0,opcode:OP_GOVERN,caller_space:LUNA_SPACE_USER as u64,domain_key:0,cid_low:0,cid_high:0,status:0,count:0,target_space:0,target_gate:0,ttl:0,uses:0,seal_low:0,seal_high:0,nonce:0,buffer_addr:(&mut protect_query as *mut LunaGovernanceQuery) as u64,buffer_size:size_of::<LunaGovernanceQuery>() as u64};govern(&mut protect_gate);if protect_gate.status==LUNA_GATE_DENIED{serial_write(b"[SECURITY] secret.protect negative ok\r\n");}else{serial_write(b"[SECURITY] secret.protect negative fail\r\n");}}
fn security_trust_selfcheck(){let mut request=EMPTY_TRUST_REQUEST;let mut gate=LunaGate{sequence:0,opcode:OP_TRUST_EVAL,caller_space:LUNA_SPACE_USER as u64,domain_key:LUNA_CAP_PACKAGE_INSTALL,cid_low:0,cid_high:0,status:0,count:0,target_space:LUNA_SPACE_SECURITY,target_gate:OP_TRUST_EVAL,ttl:0,uses:0,seal_low:0,seal_high:0,nonce:0,buffer_addr:(&mut request as *mut LunaTrustEvalRequest) as u64,buffer_size:size_of::<LunaTrustEvalRequest>() as u64};trust_eval(&mut gate);if gate.status==LUNA_GATE_DENIED{serial_write(b"[SECURITY] trust.eval negative ok\r\n");}else{serial_write(b"[SECURITY] trust.eval negative fail\r\n");}}
fn network_test_gate(caller:u64,actor:u64,domain_key:u64,cid:LunaObjectRef,target_space:u32,target_gate:u32,session:u32,channel:u32,flags:u64)->LunaGate{LunaGate{sequence:0,opcode:OP_VALIDATE_NETWORK,caller_space:caller,domain_key,cid_low:cid.low,cid_high:cid.high,status:0,count:0,target_space,target_gate,ttl:session,uses:channel,seal_low:actor,seal_high:0,nonce:flags,buffer_addr:0,buffer_size:0}}
fn network_policy_selfcheck(){let holder_cid=issue_internal_cid(LUNA_CAP_NETWORK_SEND,LUNA_SPACE_NETWORK,LUNA_NETWORK_SEND_PACKET);let mut holder_gate=network_test_gate(LUNA_SPACE_USER as u64,LUNA_SPACE_USER as u64,LUNA_CAP_NETWORK_SEND,holder_cid,LUNA_SPACE_NETWORK,LUNA_NETWORK_SEND_PACKET,0,0,0);validate_network_policy(&mut holder_gate);if holder_gate.status==LUNA_GATE_DENIED{serial_write(b"[SECURITY] network.caller negative ok\r\n");}else{serial_write(b"[SECURITY] network.caller negative fail\r\n");}let revoked_cid=issue_internal_cid(LUNA_CAP_NETWORK_SEND,LUNA_SPACE_NETWORK,LUNA_NETWORK_SEND_PACKET);if let Some(index)=find_cid(revoked_cid.low,revoked_cid.high){unsafe{let entry=*SESSION_TABLE.get_unchecked(index);tombstone_cid(entry);*SESSION_TABLE.get_unchecked_mut(index)=EMPTY_CID;}}let mut revoked_gate=network_test_gate(LUNA_SPACE_SECURITY as u64,LUNA_SPACE_SECURITY as u64,LUNA_CAP_NETWORK_SEND,revoked_cid,LUNA_SPACE_NETWORK,LUNA_NETWORK_SEND_PACKET,0,0,0);validate_network_policy(&mut revoked_gate);if revoked_gate.status==LUNA_GATE_DENIED{serial_write(b"[SECURITY] network.revoked negative ok\r\n");}else{serial_write(b"[SECURITY] network.revoked negative fail\r\n");}let offline_cid=issue_internal_cid(LUNA_CAP_NETWORK_SEND,LUNA_SPACE_NETWORK,LUNA_NETWORK_SEND_PACKET);let mut offline_gate=network_test_gate(LUNA_SPACE_SECURITY as u64,LUNA_SPACE_SECURITY as u64,LUNA_CAP_NETWORK_SEND,offline_cid,LUNA_SPACE_NETWORK,LUNA_NETWORK_SEND_PACKET,0,0,LUNA_NETWORK_POLICY_RAW_PACKET|LUNA_NETWORK_POLICY_OFFLINE);validate_network_policy(&mut offline_gate);if offline_gate.status==LUNA_GATE_DENIED{serial_write(b"[SECURITY] network.offline negative ok\r\n");}else{serial_write(b"[SECURITY] network.offline negative fail\r\n");}let mut inject_gate=network_test_gate(LUNA_SPACE_PROGRAM as u64,LUNA_SPACE_PROGRAM as u64,LUNA_CAP_DEVICE_WRITE,LunaObjectRef{low:0,high:0},LUNA_SPACE_DEVICE,LUNA_DEVICE_WRITE,0,0,LUNA_NETWORK_POLICY_RAW_PACKET|LUNA_NETWORK_POLICY_DEVICE_EXEC);validate_network_policy(&mut inject_gate);if inject_gate.status==LUNA_GATE_DENIED{serial_write(b"[SECURITY] network.inject negative ok\r\n");}else{serial_write(b"[SECURITY] network.inject negative fail\r\n");}let mut actor_gate=network_test_gate(LUNA_SPACE_SECURITY as u64,LUNA_SPACE_USER as u64,LUNA_CAP_NETWORK_SEND,LunaObjectRef{low:0,high:0},LUNA_SPACE_NETWORK,LUNA_NETWORK_SEND_CHANNEL,42,7,0);validate_network_policy(&mut actor_gate);let mut program_gate=network_test_gate(LUNA_SPACE_PROGRAM as u64,LUNA_SPACE_PROGRAM as u64,LUNA_CAP_NETWORK_SESSION,LunaObjectRef{low:0,high:0},LUNA_SPACE_NETWORK,LUNA_NETWORK_OPEN_SESSION,0,0,0);validate_network_policy(&mut program_gate);let link_revoked_cid=issue_internal_cid(LUNA_CAP_NETWORK_SEND,LUNA_SPACE_NETWORK,LUNA_NETWORK_SEND_CHANNEL);if let Some(index)=find_cid(link_revoked_cid.low,link_revoked_cid.high){unsafe{let entry=*SESSION_TABLE.get_unchecked(index);tombstone_cid(entry);*SESSION_TABLE.get_unchecked_mut(index)=EMPTY_CID;}}let mut link_revoked_gate=network_test_gate(LUNA_SPACE_SECURITY as u64,LUNA_SPACE_SECURITY as u64,LUNA_CAP_NETWORK_SEND,link_revoked_cid,LUNA_SPACE_NETWORK,LUNA_NETWORK_SEND_CHANNEL,42,7,0);validate_network_policy(&mut link_revoked_gate);let mut link_offline_gate=network_test_gate(LUNA_SPACE_SECURITY as u64,LUNA_SPACE_SECURITY as u64,LUNA_CAP_NETWORK_SEND,LunaObjectRef{low:0,high:0},LUNA_SPACE_NETWORK,LUNA_NETWORK_SEND_CHANNEL,42,7,LUNA_NETWORK_POLICY_OFFLINE);validate_network_policy(&mut link_offline_gate);if actor_gate.status==LUNA_GATE_DENIED&&program_gate.status==LUNA_GATE_DENIED&&link_revoked_gate.status==LUNA_GATE_DENIED&&link_offline_gate.status==LUNA_GATE_DENIED{serial_write(b"[SECURITY] link.policy negative ok\r\n");}else{serial_write(b"[SECURITY] link.policy negative fail\r\n");}}
fn device_test_gate(caller:u64,actor:u64,domain_key:u64,cid:LunaObjectRef,target_gate:u32,flags:u64)->LunaGate{LunaGate{sequence:0,opcode:OP_VALIDATE_DEVICE,caller_space:caller,domain_key,cid_low:cid.low,cid_high:cid.high,status:0,count:0,target_space:LUNA_SPACE_DEVICE,target_gate,ttl:0,uses:0,seal_low:actor,seal_high:0,nonce:flags,buffer_addr:0,buffer_size:0}}
fn device_policy_selfcheck(){let holder_cid=issue_internal_cid(LUNA_CAP_DEVICE_READ,LUNA_SPACE_DEVICE,LUNA_DEVICE_READ);let mut holder_gate=device_test_gate(LUNA_SPACE_USER as u64,LUNA_SPACE_USER as u64,LUNA_CAP_DEVICE_READ,holder_cid,LUNA_DEVICE_READ,LUNA_DEVICE_POLICY_READ);validate_device_policy(&mut holder_gate);if holder_gate.status==LUNA_GATE_DENIED{serial_write(b"[SECURITY] device.caller negative ok\r\n");}else{serial_write(b"[SECURITY] device.caller negative fail\r\n");}let revoked_cid=issue_internal_cid(LUNA_CAP_DEVICE_WRITE,LUNA_SPACE_DEVICE,LUNA_DEVICE_WRITE);if let Some(index)=find_cid(revoked_cid.low,revoked_cid.high){unsafe{let entry=*SESSION_TABLE.get_unchecked(index);tombstone_cid(entry);*SESSION_TABLE.get_unchecked_mut(index)=EMPTY_CID;}}let mut revoked_gate=device_test_gate(LUNA_SPACE_SECURITY as u64,LUNA_SPACE_SECURITY as u64,LUNA_CAP_DEVICE_WRITE,revoked_cid,LUNA_DEVICE_WRITE,LUNA_DEVICE_POLICY_WRITE);validate_device_policy(&mut revoked_gate);if revoked_gate.status==LUNA_GATE_DENIED{serial_write(b"[SECURITY] device.revoked negative ok\r\n");}else{serial_write(b"[SECURITY] device.revoked negative fail\r\n");}let recovery_cid=issue_internal_cid(LUNA_CAP_DEVICE_WRITE,LUNA_SPACE_DEVICE,LUNA_DEVICE_BLOCK_WRITE);let mut recovery_gate=device_test_gate(LUNA_SPACE_SECURITY as u64,LUNA_SPACE_SECURITY as u64,LUNA_CAP_DEVICE_WRITE,recovery_cid,LUNA_DEVICE_BLOCK_WRITE,LUNA_DEVICE_POLICY_WRITE|LUNA_DEVICE_POLICY_UNSAFE_WRITE|LUNA_DEVICE_POLICY_RECOVERY);validate_device_policy(&mut recovery_gate);if recovery_gate.status==LUNA_GATE_DENIED{serial_write(b"[SECURITY] device.recovery negative ok\r\n");}else{serial_write(b"[SECURITY] device.recovery negative fail\r\n");}let actor_cid=issue_internal_cid(LUNA_CAP_DEVICE_LIST,LUNA_SPACE_DEVICE,LUNA_DEVICE_LIST);let mut actor_gate=device_test_gate(LUNA_SPACE_USER as u64,LUNA_SPACE_PROGRAM as u64,LUNA_CAP_DEVICE_LIST,actor_cid,LUNA_DEVICE_LIST,LUNA_DEVICE_POLICY_CONTROL);validate_device_policy(&mut actor_gate);if actor_gate.status==LUNA_GATE_DENIED{serial_write(b"[SECURITY] device.actor negative ok\r\n");}else{serial_write(b"[SECURITY] device.actor negative fail\r\n");}let derived_cid=issue_internal_cid(LUNA_CAP_DEVICE_LIST,LUNA_SPACE_DEVICE,LUNA_DEVICE_LIST);let mut derived_gate=device_test_gate(LUNA_SPACE_SECURITY as u64,LUNA_SPACE_SECURITY as u64,0,derived_cid,LUNA_DEVICE_LIST,0);validate_device_policy(&mut derived_gate);if derived_gate.status==LUNA_GATE_OK{serial_write(b"[SECURITY] device.derived positive ok\r\n");}else{serial_write(b"[SECURITY] device.derived positive fail\r\n");}let net_cid=issue_internal_cid(LUNA_CAP_DEVICE_READ,LUNA_SPACE_DEVICE,LUNA_DEVICE_READ);let mut net_gate=device_test_gate(LUNA_SPACE_SECURITY as u64,LUNA_SPACE_SECURITY as u64,0,net_cid,LUNA_DEVICE_READ,0);net_gate.ttl=LUNA_DEVICE_ID_NET0;validate_device_policy(&mut net_gate);if net_gate.status==LUNA_GATE_DENIED{serial_write(b"[SECURITY] device.netowner negative ok\r\n");}else{serial_write(b"[SECURITY] device.netowner negative fail\r\n");}if (device_required_flags(LUNA_DEVICE_WRITE,LUNA_DEVICE_ID_DISK0,0)&LUNA_DEVICE_POLICY_UNSAFE_WRITE)!=0{serial_write(b"[SECURITY] device.diskwrite derived ok\r\n");}else{serial_write(b"[SECURITY] device.diskwrite derived fail\r\n");}}
