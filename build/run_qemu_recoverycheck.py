...
RECOVERY_IMAGE = ROOT / "lunaos_recovery.img"
BOOT1_LOG = ROOT / "qemu_recovery_boot1.log"
BOOT1_ERR = ROOT / "qemu_recovery_boot1.err.log"
BOOT2_LOG = ROOT / "qemu_recovery_boot2.log"
BOOT2_ERR = ROOT / "qemu_recovery_boot2.err.log"
...
def patch_store_dirty(image_path: Path) -> None:
    blob = bytearray(image_path.read_bytes())
    offset = STORE_BASE_LBA * 512 + SUPERBLOCK_STATE_OFFSET
    blob[offset:offset + 8] = struct.pack("<Q", STORE_STATE_DIRTY)
    image_path.write_bytes(blob)
...
def main() -> int:
    qemu = find_qemu()
    shutil.copyfile(IMAGE, RECOVERY_IMAGE)

    boot1_commands = (
        b"setup.init luna dev secret\r\n"
        b"run Notes\r\n"
    )
    patch_session_script(RECOVERY_IMAGE, boot1_commands)
    boot1 = run_qemu_once(qemu, RECOVERY_IMAGE, BOOT1_LOG, BOOT1_ERR, "readback ok")
    ...
    patch_store_dirty(RECOVERY_IMAGE)
    ...
    patch_session_script(RECOVERY_IMAGE, boot2_commands)
    boot2 = run_qemu_once(qemu, RECOVERY_IMAGE, BOOT2_LOG, BOOT2_ERR, "browser surface online")
    ...
    require(boot2, "lafs.last-repair: replay", "boot2")
