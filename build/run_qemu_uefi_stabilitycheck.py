...
LOG_PATH = ROOT / "qemu_uefi_stabilitycheck.log"
ERR_PATH = ROOT / "qemu_uefi_stabilitycheck.err.log"
DEBUG_PATH = ROOT / "qemu_uefi_stabilitycheck.debug.log"
VARS_PATH = ROOT / "ovmf-uefi-stability-vars.fd"
...
def main() -> int:
    ...
    if "LunaLoader UEFI Stage 1 handoff" not in stdout:
        raise RuntimeError("UEFI stabilitycheck did not reach BOOTX64.EFI handoff output before timeout")
    if "[BOOT] dawn online" not in stdout:
        raise RuntimeError("UEFI stabilitycheck did not reach BOOT runtime before timeout")
    if "[SYSTEM] Space 14 ready." not in stdout:
        raise RuntimeError("UEFI stabilitycheck did not reach the full space bring-up before timeout")
    if "[USER] shell ready" not in stdout:
        raise RuntimeError("UEFI stabilitycheck did not reach the Luna shell before timeout")
    if "Exception Type" in stdout or "Can't find image information" in stdout:
        raise RuntimeError("UEFI stabilitycheck encountered a firmware exception dump")
